...
architecture arch_imp of noip_lvds_stream is
	...
	signal lvds_sync_word : std_logic_vector(SENSOR_BIT_LENGTH-1 downto 0);
	signal lvds_data_word : std_logic_vector((4*SENSOR_BIT_LENGTH)-1 downto 0);
...
noip_lvds_stream_master_stream_v1_0_M00_AXIS_inst : noip_lvds_stream_master_stream_v1_0_M00_AXIS
	...
	lvds_data_word <= lvds_deserialized((5*SENSOR_BIT_LENGTH)-1 downto SENSOR_BIT_LENGTH);
			...
			case DState is
				...
				when REC_IMG => -- receiving pixels and storing them. 
					...
					if(lvds_sync_word = LINE_END) then
						...
						-- 	kernel(pixel_polarity) <= lvds_data_word(0);
						-- 	kernel(pixel_polarity+2) <= lvds_data_word(1);
						-- 	kernel(pixel_polarity+4) <= lvds_data_word(2);
						-- 	kernel(pixel_polarity+6) <= lvds_data_word(3);
						-- else 								 -- odd kenel : pixels read out descending
						-- 	kernel(7-pixel_polarity) <= lvds_data_word(0);
						-- 	kernel(5-pixel_polarity) <= lvds_data_word(1);
						-- 	kernel(3-pixel_polarity) <= lvds_data_word(2);
						-- 	kernel(1-pixel_polarity) <= lvds_data_word(3);
						-- end if; 
