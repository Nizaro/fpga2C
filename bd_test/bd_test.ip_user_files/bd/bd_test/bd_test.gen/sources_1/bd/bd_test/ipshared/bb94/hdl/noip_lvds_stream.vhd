...
entity noip_lvds_stream is
	generic (
		...
		SENSOR_BIT_LENGTH : integer := 8;
		IM_WIDTH : integer := 1280;
		IM_HEIGHT : integer := 1024;
	...
	port (
		...
		lvds_deserialized : in std_logic_vector((5*SENSOR_BIT_LENGTH)-1 downto 0);
		-- lvds_sync_word : in std_logic_vector(SENSOR_BIT_LENGTH-1 downto 0);
		-- lvds_data_word : in std_logic_vector((4*SENSOR_BIT_LENGTH)-1 downto 0);
...
noip_lvds_stream_master_stream_v1_0_M00_AXIS_inst : noip_lvds_stream_master_stream_v1_0_M00_AXIS
	...
	lvds_sync_word <= lvds_deserialized(SENSOR_BIT_LENGTH-1 downto 0);
	-- for p in 0 to 3 loop
	-- 	lvds_data_words(p) <= (((p+2)*SENSOR_BIT_LENGTH)-1 downto (p+1)*SENSOR_BIT_LENGTH);
	-- end loop;

	lvds_fsm : process(lvds_clk_div, s00_axis_aresetn) 
	begin
		if(s00_axis_aresetn = '0') then
			...
			image_start <= '0';
			image_end <= '0';
		elsif(rising_edge(lvds_clk_div)) then
			case DState is
				when ALIGNING =>
					if(lvds_sync_word = TR) then -- training pattern found : alignment done.
						DState <= IDLE;
					else
						bitslip <= std_logic_vector(to_unsigned(findbitslip(lvds_sync_word,TR),4));
					end if;
				...
				when REC_IMG => -- receiving pixels and storing them. 
					...
					if(lvds_sync_word = LINE_END) then
						...
						if((nb_kernel rem 2) = 0) then	 	 -- even kernel : pixels read out ascending
							kernel(pixel_polarity) <= lvds_deserialized((2*SENSOR_BIT_LENGTH)-1 downto SENSOR_BIT_LENGTH);
							kernel(pixel_polarity+2) <= lvds_deserialized((3*SENSOR_BIT_LENGTH)-1 downto 2*SENSOR_BIT_LENGTH);
							kernel(pixel_polarity+4) <= lvds_deserialized((4*SENSOR_BIT_LENGTH)-1 downto 3*SENSOR_BIT_LENGTH);
							kernel(pixel_polarity+6) <= lvds_deserialized((5*SENSOR_BIT_LENGTH)-1 downto 4*SENSOR_BIT_LENGTH);
						else 								 -- odd kernel : pixels read out descending
							kernel(7-pixel_polarity) <= lvds_deserialized((2*SENSOR_BIT_LENGTH)-1 downto SENSOR_BIT_LENGTH);
							kernel(5-pixel_polarity) <= lvds_deserialized((3*SENSOR_BIT_LENGTH)-1 downto 2*SENSOR_BIT_LENGTH);
							kernel(3-pixel_polarity) <= lvds_deserialized((4*SENSOR_BIT_LENGTH)-1 downto 3*SENSOR_BIT_LENGTH);
							kernel(1-pixel_polarity) <= lvds_deserialized((5*SENSOR_BIT_LENGTH)-1 downto 4*SENSOR_BIT_LENGTH);
						end if; 
