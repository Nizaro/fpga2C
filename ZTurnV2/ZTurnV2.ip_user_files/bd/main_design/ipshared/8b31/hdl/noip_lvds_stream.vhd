		...
		if(s00_axis_aresetn = '0') then
			...
			if(i = SENSOR_BIT_LENGTH-1) then
				...
				if(bitslip > 0 and bitslip < SENSOR_BIT_LENGTH and temp_sync_word /= TR) then
					-- lvds_sync_word <= temp_sync_word(SENSOR_BIT_LENGTH-bitslip-1 downto 0) & temp_sync_word(SENSOR_BIT_LENGTH-1 downto SENSOR_BIT_LENGTH-bitslip);
					-- for c in 0 to 3 loop
					-- 	lvds_data_words(c) <= temp_data_words(c)(SENSOR_BIT_LENGTH-bitslip-1 downto 0) & temp_data_words(c)(SENSOR_BIT_LENGTH-1 downto SENSOR_BIT_LENGTH-bitslip);
					-- end loop;
					i <= bitslip;
					lvds_sync_word <= temp_sync_word;
					lvds_data_words <= temp_data_words;
					temp_sync_word(bitslip downto 0) := TR(bitslip downto 0);
				else
					i <= 0;
					lvds_sync_word <= temp_sync_word;
					lvds_data_words <= temp_data_words;
